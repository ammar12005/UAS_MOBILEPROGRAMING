            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      _buildStatCard('$totalTickets', 'Total', Icons.confirmation_number, const Color(0xFF3B82F6)),
                      const SizedBox(width: 12),
                      _buildStatCard('$scannedTickets', 'Hadir', Icons.check_circle, const Color(0xFF10B981)),
                      const SizedBox(width: 12),
                      _buildStatCard('$remainingTickets', 'Sisa', Icons.pending, const Color(0xFFF59E0B)),
                    ],
                  ),
                ],
              ),
                        ...
                        children: [
                          const Row(
                            children: [
                              ...
                              SizedBox(width: 12),
                              Text('Input Manual', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                            ],
                          ),
                          const SizedBox(height: 16),
                          Row(
                            ...
                            children: [
                              const Row(
                                children: [
                                  ...
                                  SizedBox(width: 12),
                                  Text('Scan QR Code', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                                ],
                              ),
                              IconButton(
                                icon: Icon(
                                  _isCameraActive ? Icons.videocam : Icons.videocam_off,
                                  color: _isCameraActive ? const Color(0xFF10B981) : Colors.red,
                                ),
                                onPressed: () {
                                  setState(() {
                                      ...
                                      child: Center(
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            const SizedBox(height: 16),
                                            const Text('Kamera Dimatikan', style: TextStyle(color: Colors.white54, fontSize: 16)),
                                          ],
                                        ),
                                      ),
                                    ),
  ...
  Widget _buildResultCard() {
    ...
    final event = result.ticket != null
        ? widget.events.firstWhere(
            ...
            orElse: () => Event(id: 0, name: 'Unknown Event', date: DateTime.now(), location: '', capacity: 0, price: 0, description: ''),
          )
        : null;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(20),
        boxShadow: [BoxShadow(color: baseColor.withValues(alpha: 0.3), blurRadius: 20, offset: const Offset(0, 10))],
      ),
      child: Column(
        children: [
          Row(
            children: [
              Container(
                ...
                child: Icon(result.isValid ? Icons.check_circle : Icons.error, color: Colors.white, size: 32),
              ),
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(result.isValid ? 'BERHASIL!' : 'GAGAL!', style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 4),
                    Text(result.message, style: TextStyle(color: Colors.white.withValues(alpha: 0.9), fontSize: 14)),
                  ],
                ),
              ),
            ],
          ...
          if (result.ticket != null) ...[
            ...
            Container(
              ...
              child: Column(
                children: [
                  if (result.isValid) ...[
                    Text('Selamat Datang! 🎉', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: baseColor)),
                    const SizedBox(height: 16),
                  ],
                  _buildInfoRow(Icons.person, 'Nama', result.ticket!.buyerName),
                  const SizedBox(height: 8),
                  _buildInfoRow(Icons.confirmation_number, 'Kode', result.ticket!.code),
                  const SizedBox(height: 8),
                  if (event != null) _buildInfoRow(Icons.event, 'Event', event.name),
                  if (result.ticket!.scannedAt != null) ...[
                    const SizedBox(height: 8),
                    _buildInfoRow(Icons.access_time, 'Check-in', DateFormat('HH:mm:ss').format(result.ticket!.scannedAt!)),
                  ],
                ],
              ),
            ),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: const Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [Icon(Icons.qr_code_scanner), SizedBox(width: 12), Text('SCAN LAGI', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold))],
              ),
            ),
          ),
        ],
